...
if df is not None:
    ...
    if ('Hours_Studied' in df_clean.columns and
        ...
        def create_study_sleep_group(row):
            ...
            if high_study and high_sleep:
                ...
                return 'Low Both'
        study_sleep_groups = df_clean.apply(
            create_study_sleep_group, axis=1
        )
        df_clean['Study_Sleep_Group'] = study_sleep_groups
    # Sidebar navigation
    ...
    if page == "📋 Overview":
        ...
        - **H2**: Study hours + attendance together predict better than either
        - **H3**: Students with high study AND high sleep score
        ...
        # Finding 1
        with st.expander(
            "Finding 1: Study Hours Dominate Individual Predictors",
            expanded=True
        ):
            st.write("""
            **What We Found**: Hours studied explained **19.8%**
            (R² = 0.198) of the variance in exam scores, making it
            the only meaningful individual predictor. In contrast,
            attendance and sleep hours alone showed virtually no
            predictive power.
            **Why It Matters**: When students ask "What's the most
            important thing I can do to improve my grades?" — the
            data clearly points to **study time**.
            """)
        ...
        with st.expander(
            ...
            st.write("""
            **What We Found**: While attendance alone doesn't predict
            scores, it plays a crucial **moderating role**. The
            relationship between study hours and exam scores changes
            depending on attendance levels.

            **Why It Matters**: **Showing up to class amplifies the
            effectiveness of studying**. This means attendance
            policies aren't just about being present—they directly
            impact how well study time translates to results.
            """)
        # Finding 3
        expander_title = "Finding 3: The 'Sweet Spot' for Student Success"
        with st.expander(expander_title, expanded=True):
            st.write(
                "**What We Found**: Students were grouped into "
                "four categories:"
            )
            # Group data from your notebook
            group_data = pd.DataFrame({
                'Group': [
                    'High Both', 'High Study Only',
                    'High Sleep Only', 'Low Both'
                ],
            ...
            **Statistical Significance**:
            ...
            - Tukey HSD confirms High Study Only and High Both groups
              score significantly higher
            **Why It Matters**: **Study time drives success, not sleep
            alone**. Students who study a lot score ~3 points higher
            regardless of whether they sleep a lot. However, the
            **High Both group is the largest**, suggesting successful
            students tend to maintain both habits.
            """)
        # Finding 4
        with st.expander(
            ...
            st.write(
                "**What We Found**: When comparing all factors, "
                "here's how they rank:"
            )
            rank_data = pd.DataFrame({
                'Rank': ['🥇', '🥈', '🥉', '4', '5', '6'],
                'Factor': [
                    'Attendance', 'Hours Studied', 'Previous Scores',
            ...
            st.write("""
            **Why It Matters**: This reveals something fascinating!
            While Hours Studied was the best individual predictor,
            **Attendance actually has a stronger correlation** with
            exam scores (0.58 vs 0.45). This tells us that
            attendance and study hours work together—students who
            attend class regularly also tend to study more.
            """)
    elif page == "📈 Visualizations":
        st.header("📈 Visualizations")
        tab1, tab2, tab3 = st.tabs([
            "Correlation Matrix", "Group Comparison", "Predictor Ranking"
        ])
        with tab1:
            ...
            if len(numerical_df.columns) > 1:
                ...
                if 'Exam_Score' in numerical_df.columns:
                    ...
                    exam_corr = (corr_matrix['Exam_Score']
                                 .drop('Exam_Score')
        ...
        with tab2:
            ...
            if ('Study_Sleep_Group' in df_clean.columns and
               'Exam_Score' in df_clean.columns):
                fig, ax = plt.subplots(figsize=(10, 6))
                sns.boxplot(data=df_clean, x='Study_Sleep_Group',
                            y='Exam_Score', ax=ax)
        ...
        with tab3:
            ...
            for bar, val in zip(bars, correlations):
                ...
                ax.text(bar.get_x() + bar.get_width()/2., height,
                        f'{val:.2f}', ha='center', va=va_pos)
            ax.set_title(
                'Correlation of Variables with Exam Score',
                fontsize=14
            )
        ...
        **Technologies Used**:
        ...
        - Streamlit (Dashboard)
        **Author**: Sadiyah
        **GitHub**: [symemi115/Student-Performance-Factors](
            https://github.com/symemi115/Student-Performance-Factors)
        """)
else:
    st.error("""
    **Data file not found!**
